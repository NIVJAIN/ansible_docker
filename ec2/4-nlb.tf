...
resource "aws_lb_target_group" "tg" {
  for_each = local.forwarding_config
    #this give error of 32 characters long # name = "${lookup(local.nlb_config, "environment")}-tg-${lookup(local.tg_config, "name")}-${each.key}"
    name                  = "${lookup(local.tg_config, "name")}-${each.key}"
    port                  = each.key
    protocol              = each.value
